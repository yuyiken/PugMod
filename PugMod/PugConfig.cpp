...
CPugConfig gPugConfig;

void CPugConfig::ServerActivate()
{
...
void CPugConfig::Execute(std::string File)
{
	if (!File.empty())
	{
		if (!this->m_Path.empty())
		{
			gPugUtil.ServerCommand("exec %s/%s", this->m_Path.c_str(), File.c_str());
		}
	}
}

std::string CPugConfig::GetFullPath()
{
