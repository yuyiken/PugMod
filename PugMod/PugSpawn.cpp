...
void CPugSpawn::LoadSpawns()
{
    this->m_Spawns.clear();

    char Path[MAX_PATH] = { 0 };

    Q_sprintf(Path, "%s/spawns/%s.txt", gPugConfig.GetFullPath().c_str(), STRING(gpGlobals->mapname));
...
std::map<size_t, P_SPAWN_POINT> CPugSpawn::GetSpawns()
{
    return this->m_Spawns;
}

void CPugSpawn::Enable()
{
    if (!this->m_Enabled)
    {
        this->LoadSpawns();
...
bool CPugSpawn::CheckDistance(CBasePlayer* Player, vec3_t Origin, float Distance)
{
