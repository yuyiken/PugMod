...
void CPugCurl::ServerDeactivate()
{
			...
			do
			{
				...
				while (MsgInfo = curl_multi_info_read(this->m_MultiHandle, &HandleCount))
				{
					if (MsgInfo->msg == CURLMSG_DONE)
					{
						...
						curl_easy_getinfo(MsgInfo->easy_handle, CURLINFO_PRIVATE, &RequestIndex);

						if (this->m_Data.find(RequestIndex) != this->m_Data.end())
						{
							if (this->m_Data[RequestIndex].Callback)
							{
								((void(*)(CURL*, size_t, const char*, long))this->m_Data[RequestIndex].Callback)(MsgInfo->easy_handle, this->m_Data[RequestIndex].Size, this->m_Data[RequestIndex].Memory, this->m_Data[RequestIndex].CallbackData);
							}

							this->m_Data.erase(RequestIndex);
						}
...
void CPugCurl::PostJSON(const char* url, long Timeout, std::string BearerToken, std::string PostData, void* FunctionCallback, long CallbackData)
{
	if (this->m_ServerActive)
	{
		if (this->m_MultiHandle)
