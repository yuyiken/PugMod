...
int CPugMod::SetState(int State)
{
    ...
    switch(State)
    {
        ...
        case STATE_KNIFE_ROUND:
        {
            ...
            gPugUtil.PrintColor(nullptr, E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 Round Faca: O ^3vencedor escolhe o time inicial."), gPugCvar.m_Tag->string);
        ...
        case STATE_FIRST_HALF:
        {
            ...
            gPugUtil.PrintColor(nullptr, E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 ^3%s^1 Iniciado: Prepare-se !!"), gPugCvar.m_Tag->string, g_Pug_String[State]);
...
void CPugMod::SwapTeams()
{
    ...
    gPugUtil.PrintColor(nullptr, E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 Trocando times automaticamente."), gPugCvar.m_Tag->string);
}
...
bool CPugMod::ChooseTeam(CBasePlayer *Player, int Slot)
{
    ...
    {
        gPugUtil.PrintColor(Player->edict(), E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 Seleção automática não permitida."), gPugCvar.m_Tag->string);
        return true;
    }
    ...
    if (Slot == MENU_SLOT_TEAM_SPECT)
    {
        ...
        {
            gPugUtil.PrintColor(Player->edict(), E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 Espectadores não permitidos."), gPugCvar.m_Tag->string);
            return true;
        }
    }
    ...
    {
        gPugUtil.PrintColor(Player->edict(), E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 Você já está no time ^3%s^1."), gPugCvar.m_Tag->string, g_Pug_TeamName[Player->m_iTeam]);
        return true;
    }
    ...
    if (Player->m_iTeam == TERRORIST || Player->m_iTeam == CT)
    {
        ...
        {
            gPugUtil.PrintColor(Player->edict(), E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 Troca de time não permitida durante a partida."), gPugCvar.m_Tag->string);
            return true;
        }
    }
	...
	if (Slot == MENU_SLOT_TEAM_TERRORIST || Slot == MENU_SLOT_TEAM_CT)
	{
		...
		{
			gPugUtil.PrintColor(Player->edict(), E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 O time ^3%s^1 já está completo."), gPugCvar.m_Tag->string, g_Pug_TeamName[Slot]);
			return true;
		}
	}
        ...
        if (!Player->IsBot())
        {
            if (Player->m_iTeam == TERRORIST || Player->m_iTeam == CT)
            {
                if (Player->m_iLastAccount < 1)
                {
                    ...
                    gPugUtil.PrintColor(Player->edict(), E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 Digite ^3%shelp^1 para lista de comandos."), gPugCvar.m_Tag->string, gPugCvar.m_CmdPrefixPlayer->string);
                }
            }
        }
    }
    ...
    if (this->m_State >= STATE_KNIFE_ROUND && this->m_State <= STATE_OVERTIME)
    {
        ...
        else
        {
            ...
            if (Player)
            {
                if (Player->m_iTeam == TERRORIST || Player->m_iTeam == CT)
                {
                    gPugUtil.PrintColor(nullptr, E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 ^3%s^1 Saiu da partida.."), gPugCvar.m_Tag->string, STRING(pEntity->v.netname));
                }
            }
...
void CPugMod::RoundStart()
{
    if (this->m_State == STATE_FIRST_HALF || this->m_State == STATE_SECOND_HALF || this->m_State == STATE_OVERTIME)
    {
        if (g_pGameRules)
        {
            ...
            gPugUtil.ClientPrint(nullptr, E_PRINT::CONSOLE, _T("[%s] Round %d Iniciado."), gPugCvar.m_Tag->string, this->GetRound() + 1);
        }
    }
}
        ...
        if (winStatus == WINSTATUS_CTS || winStatus == WINSTATUS_TERRORISTS)
        {
            if (g_pGameRules)
            {
                if (!CSGameRules()->m_bCompleteReset)
                {
                    ...
                    gPugUtil.ClientPrint(nullptr, E_PRINT::CONSOLE, _T("[%s] Round %d Ganho Por: %s."), gPugCvar.m_Tag->string, (this->GetRound() + 1), g_Pug_TeamName[Winner]);
...
void CPugMod::Status(CBasePlayer *Player)
{
    ...
    gPugUtil.PrintColor(Player->edict(), Player->entindex(), _T("^4[%s]^1 Status: ^3%s^1"), gPugCvar.m_Tag->string, g_Pug_String[this->m_State]);
    ...
    if (g_pGameRules)
    {
        ...
        if (this->m_State >= STATE_FIRST_HALF && this->m_State <= STATE_END)
        {
            ...
            if (gPugCvar.m_ScoreText->value)
            {
                ...
                {
                    gPugUtil.PrintColor(pEntity, Sender, _T("^4[%s]^1 Fim de jogo: ^3%s^1 (^4%d^1) - (^4%d^1) ^3%s^1"), gPugCvar.m_Tag->string, g_Pug_TeamName[TERRORIST], ScoreTR, g_Pug_TeamName[CT], ScoreCT);
                }
            }
            else
            {
                if (ScoreTR != ScoreCT)
                {
                    ...
                    if (this->m_State != STATE_END)
                    {
                        gPugUtil.PrintColor(pEntity, Sender, _T("^4[%s]^1 Os ^3%s^1 estão vencendo: %d-%d"), gPugCvar.m_Tag->string, g_Pug_TeamName[Winner], this->GetScore(Winner), this->GetScore(Losers));
                    }
                    else
                    {
                        gPugUtil.PrintColor(pEntity, Sender, _T("^4[%s]^1 Fim de jogo! Os ^3%s^1 venceram: %d-%d"), gPugCvar.m_Tag->string, g_Pug_TeamName[Winner], this->GetScore(Winner), this->GetScore(Losers));
                    }
                }
                else
                {
                    if (this->m_State != STATE_END)
                    {
                        gPugUtil.PrintColor(pEntity, Sender, _T("^4[%s]^1 Placar empatado: %d-%d"), gPugCvar.m_Tag->string, ScoreTR, ScoreCT);
                    }
                    else
                    {
                        gPugUtil.PrintColor(pEntity, Sender, _T("^4[%s]^1 Fim de jogo! Placar empatado: %d-%d"), gPugCvar.m_Tag->string, ScoreTR, ScoreCT);
                    }
                }
            }
        }
        ...
        {
            gPugUtil.PrintColor(pEntity, E_PRINT_TEAM::DEFAULT, _T("^4[%s]^1 Comando indisponível."), gPugCvar.m_Tag->string);
        }
    }
}
